...
#if defined(NDEBUG)
# error "Djoshcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Djoshcoin Signed Message:\n";
...
{
    // Djoshcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 2.5 * COIN;

    // Subsidy is cut in half every 4200000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 4200000); // Djoshcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60; // Djoshcoin: 1 hour
static const int64 nTargetSpacing = .5 * 60; // Djoshcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Djoshcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Djoshcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Do you know da wae. Somebody Toucha My Spaghet.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2.5 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0443c99fadd486b0301f3e5079de79c28f64378ee8531476cf966366b242726e2be0156e3cb1bd865b429aa35cb6b990ee6d6d9155685a9963117959ec9e9cbef6") << OP_CHECKSIG;
        CBlock block;
        ...
        assert(block.hashMerkleRoot == uint256("0x2d7fb0affe7fc97fa1654853ed8a43bca167e4aefca059464b7e4c2cbff65ce0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf9, 0xc4, 0xb8, 0xdc }; // Djoshcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DjoshcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DjoshcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DjoshcoinMiner(CWallet *pwallet)
{
    printf("DjoshcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("djoshcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DjoshcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DjoshcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DjoshcoinMiner, pwallet));
}
